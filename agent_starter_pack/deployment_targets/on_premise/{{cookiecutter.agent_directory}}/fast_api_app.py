...
# On-Premise Deployment FastAPI Application
# This version uses local services instead of Google Cloud
            ...
            try:
                ...
                if "text" in message:
                    ...
                    if isinstance(data, dict):
                        ...
                        if "setup" in data:
                            logger.info(f"Setup message: {data['setup']}")
                            logging.info(
...
{%- endif %}
from {{cookiecutter.agent_directory}}.app_utils.storage import ensure_storage_initialized
...
# Initialize storage (local filesystem or MinIO)
storage_path = ensure_storage_initialized()
logger.info(f"Storage initialized at: {storage_path}")

{%- if cookiecutter.is_a2a %}

# Initialize artifact service based on storage type
storage_type = os.getenv("STORAGE_TYPE", "local")
if storage_type == "local":
    from google.adk.artifacts.local_artifact_service import LocalArtifactService
    artifact_service = LocalArtifactService(base_path=storage_path)
else:
    # For MinIO/S3-compatible storage
    from {{cookiecutter.agent_directory}}.app_utils.artifacts import get_artifact_service
...
# Session service configuration from environment
session_service_uri = os.environ.get("DATABASE_URL")

# Use artifact service URI based on storage type
artifact_service_uri = storage_path
...
app: FastAPI = get_fast_api_app(
    ...
    otel_to_cloud=False,  # Use local OpenTelemetry exporters
)
